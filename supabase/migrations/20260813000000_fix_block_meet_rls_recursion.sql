-- Keep block-aware meet access behind a security-definer function so meets and
-- meet_attendees do not recursively evaluate each other's RLS policies.
...
as $$
  select exists (
    ...
    where m.id = p_meet_id
      and not public.is_user_blocked(m.created_by)
      and (
        m.is_public = true
